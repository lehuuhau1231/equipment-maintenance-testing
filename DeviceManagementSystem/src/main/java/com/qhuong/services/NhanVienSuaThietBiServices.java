...
public class NhanVienSuaThietBiServices {
    public List<NhanVienSuaThietBi> getNhanVienSuaThietBi() throws SQLException {
        List<NhanVienSuaThietBi> repair = new ArrayList<>();
        try (Connection conn = JdbcUtils.getConn()) {
            PreparedStatement stm = conn.prepareCall("SELECT b.id, b.ngaySua, t.tenThietBi, nv.tenNV "
                    + "FROM nhanviensuathietbi b "
                    + "JOIN ThietBi t ON b.idThietBi = t.id "
                    + "JOIN NhanVienSuaChua nv ON b.idNhanVien = nv.id");
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                NhanVienSuaThietBi r = new NhanVienSuaThietBi(rs.getInt("id"), rs.getTimestamp("ngaySua").toLocalDateTime(), rs.getString("tenThietbi"), rs.getString("tenNV"));
                repair.add(r);
            }
        }
        return repair;
    ...
    public List<LocalDateTime> getListDateTime(int idNhanVien) throws SQLException {
        ...
        try (Connection conn = JdbcUtils.getConn()) {
            PreparedStatement stm = conn.prepareCall("SELECT ngaySua FROM nhanviensuathietbi WHERE idNhanVien=?");
